...
$Search_List.="<A target=\"tags\" HREF=\"".$Search_link."search?key=".urlencode($Search_Key1[$j])."\">".$kname."</A> ";
}
}else{
$kname=(empty($name))?$Search_Key:$name;
$Search_List="<A target=\"tags\" HREF=\"".$Search_link."search?key=".urlencode($Search_Key)."\">".$kname."</A> ";
}
//开启二级域名
$Ym_Mode=config('Ym_Mode','vod'); //二级域名状态
$Ym_Url=config('Ym_Url','vod');   //二级域名地址

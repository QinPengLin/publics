...
//判断是否收藏
$rows=$this->db->query("SELECT id FROM ".CS_SqlPrefix."vod_fav where did=".$id." and uid=".$_SESSION['mscms__id']." and sid=0")->row();
if($rows){
$error='您已经收藏了该视频';
}else{
$add['did']=$id;
...
//踩
public function vodcai()
{
$callback = $this->input->get('callback',true);
$id = intval($this->uri->segment(3));   //ID
$cai = $this->cookie->get_cookie("vodcai_id_".$id);
if($id==0){
$error='ID为空';
//}elseif(!$this->CsdjUser->User_Login(1)){
//$error='您还没有登录';
...
$row=$this->CsdjDB->get_row('vod','chits',$id);
if(!$row){
$error='数据不存在';
}else{
//记住cookie
$this->cookie->set_cookie("vodcai_id_".$id,'ok',time()+86400);
//增加顶人气
$updata['chits']=$row->dhits+1;
$this->CsdjDB->get_update('vod',$id,$updata);
$error='ok';
}
}

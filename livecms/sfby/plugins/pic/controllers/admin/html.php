...
class Html extends Mscms_Controller {
	...
	public function type_save()
	{
            if($this->huri['lists']['check']==0){
				admin_msg('图册分类未开启生成~!','javascript:history.back();','no');
			}
	...
	public function show_save()
	{
            if($this->huri['show']['check']==0){
				admin_msg('图册内容页未开启生成~!','javascript:history.back();','no');
			}
            ...
            echo '<LINK href="'.base_url().'packs/admin/css/style.css" type="text/css" rel="stylesheet"><br>';
			echo '&nbsp;&nbsp;<b>正在开始生成图册内容,分<font color=red>'.$pagejs.'</font>次生成，当前第<font color=red>'.$page.'</font>次</b><br/>';
            ...
            foreach ($query->result_array() as $row) {
                  ...
                  //生成
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "&nbsp;<font style=font-size:10pt;>生成图册:<font color=red>".$row['name']."</font>成功:<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/>";
                  ob_flush();flush();

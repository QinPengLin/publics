...
class Type extends Mscms_Controller {
	...
	public function save()
	{
            ...
            if(empty($name)||empty($data['cid'])){
                   admin_msg('抱歉，图册名称、分类不能为空！','javascript:history.back();','no');
			}
			...
			if($this->db->table_exists(CS_SqlPrefix.'singer')){  //演员表存在
	...
	public function del()
	{
			...
			if($ac=='hui'){
			     ...
			     foreach ($result as $row) {
                     ...
                     if(!empty($row->pic)){
					    $this->csup->del($row->pic,'pic'); //删除图册
				     }
	...
	public function pl_save()
	{
            ...
            if($xid==1){  //按ID操作
				   if(empty($id)) admin_msg('请选择要操作的图册ID！','javascript:history.back();','no');
                   foreach ($csid as $v) {
                          if($v=="cid"){
				   ...
				   if(empty($cids)) admin_msg('请选择要操作的图册分类！','javascript:history.back();','no');
                   foreach ($csid as $v) {
                          if($v=="cid"){
	...
	public function dt($id,$sid=0)
	{
			...
			if($dt){
				  ...
				  if($sid>0){ //删除回收站
				      ...
				      $add['uidb']=0;
				      $add['name']='图册被删除';
				      $add['neir']='您的图册《'.$dt->name.'》被删除，系统同时扣除您'.User_Cion_Del.'个金币，'.User_Jinyan_Del.'个经验';
				      $add['addtime']=time();
				      ...
				      $add['uidb']=0;
				      $add['name']='图册审核成功通知';
				      $add['neir']='恭喜您，新建的图册 <b>['.$dt->name.']</b> 符合标准，审核通过，赶快去上传图片吧！'.$str.'';
				      $add['addtime']=time();
            	      $this->CsdjDB->get_insert('msg',$add);
				  }
			}

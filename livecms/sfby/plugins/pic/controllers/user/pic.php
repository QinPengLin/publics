...
$sqlstr = "select {field} from ".CS_SqlPrefix."pic where yid=".$yid." and uid=".$_SESSION['mscms__id'];
if($cid>0){
$cids=getChild($cid);
...
//URL地址
$url='pic/type/'.$cid;
$sqlstr = "select {field} from ".CS_SqlPrefix."pic_type where yid=0 and uid=".$_SESSION['mscms__id'];
if($cid>0){
$cids=getChild($cid);
$sqlstr.= " and cid in(".$cids.")";
}
...
//URL地址
$url='pic/verify/'.$cid;
$sqlstr = "select {field} from ".CS_SqlPrefix."pic_type where yid=1 and uid=".$_SESSION['mscms__id'];
if($cid>0){
$cids=getChild($cid);
$sqlstr.= " and cid in(".$cids.")";
}
...
if(!$rowc || $rowc->uid!=$_SESSION['mscms__id']) msg_url('相册不存在~!','javascript:history.back();');
$name=$rowc->name;
}
//模板
$tpl='add.html';
...
$ids=implode(',', $id);
$this->db->query("delete from ".CS_SqlPrefix."pic where uid=".$_SESSION['mscms__id']." and id in(".$ids.")");
}else{ //按ID
$this->db->query("delete from ".CS_SqlPrefix."pic where uid=".$_SESSION['mscms__id']." and id=".$id."");
}
...
//URL地址
$url='pic/res/'.$sid;
$sqlstr = "SELECT * FROM ".CS_SqlPrefix."pic_type where yid=0 and hid=0 and uid=".$_SESSION['mscms__id'];
//当前会员
$row=$this->CsdjDB->get_row_arr('user','*',$_SESSION['mscms__id']);
if(empty($row['nichen'])) $row['nichen']=$row['name'];
//装载模板

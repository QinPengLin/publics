...
class Show extends Mscms_Controller {
...
if(!$login) exit("alert('抱歉，该小说需要登录才能阅读，请先登录！');");
$rowu=$this->CsdjDB->get_row_arr('user','zid,zutime,vip,level,cion',$_SESSION['mscms__id']);
if($rowu['zutime']<time()){
...
if($row['vip']>$rowu['zid']){
exit("alert('抱歉，您所在的会员组不能阅读该小说，请先升级！');");
}
}
...
//判断是否下载过
$did=$id;
$rowd=$this->db->query("SELECT id,addtime FROM ".CS_SqlPrefix."news_look where did='".$did."' and uid='".$_SESSION['mscms__id']."'")->row_array();
if($rowd){
$down=1; //数据已经存在
...
echo "var mscms_content='".escape($row['content'])."';$('#mscms_content').html(unescape(mscms_content));";
}
}

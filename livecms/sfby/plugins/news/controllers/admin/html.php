 ...
 * @Dtime:2014-11-21
 */
class Html extends Mscms_Controller {
	...
	public function topic_save()
	{
            ...
            if($datacount==0){
				 ...
				 $pagesize=10;
			     preg_match_all('/{mscms:([\S]+)\s+(.*?pagesize=\"([\S]+)\".*?)}([\s\S]+?){\/mscms:\1}/',$template,$page_arr);
            ...
            for($i=$start;$i<=$pagejs;$i++){
				  ...
				  $Mark_Text=$this->CsdjTpl->plub_list(array(),1,'lists',$page,'',true,'topic.html','topic','','小说专题');
	...
	public function topicshow_save()
	{
            ...
            foreach ($query->result_array() as $row) {
                  ...
                  //生成
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "&nbsp;<font style=font-size:10pt;>生成小说专题:<font color=red>".$row['name']."</font>成功:<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/>";
                  ob_flush();flush();
	...
	public function type_save()
	{
            if($this->huri['lists']['check']==0){
				admin_msg('小说分类未开启生成~!','javascript:history.back();','no');
			}
	...
	public function show_save()
	{
            if($this->huri['show']['check']==0){
				admin_msg('小说内容页未开启生成~!','javascript:history.back();','no');
			}
            ...
            echo '<LINK href="'.base_url().'packs/admin/css/style.css" type="text/css" rel="stylesheet"><br>';
			echo '&nbsp;&nbsp;<b>正在开始生成小说内容,分<font color=red>'.$pagejs.'</font>次生成，当前第<font color=red>'.$page.'</font>次</b><br/>';
            ...
            foreach ($query->result_array() as $row) {
				  ...
				  if($row['vip']>0 || $row['level']>0 || $row['cion']>0){
				       $content="<div id='mscms_content'></div>";
                  ...
                  //生成
			      write_file(FCPATH.$Htmllink,$Mark_Text);
				  echo "&nbsp;<font style=font-size:10pt;>生成小说:<font color=red>".$row['name']."</font>成功:<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/>";
                  ob_flush();flush();

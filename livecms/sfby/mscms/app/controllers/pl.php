...
class Pl extends Mscms_Controller {
	...
	public function add()
	{
			...
			if(Pl_Modes==3){
                 ...
                 $error='10003';
			}elseif(Pl_Youke==0 && empty($_SESSION['mscms__id'])){//关闭游客评论
                 $error='10004';
			}else{

                $add['uid']=isset($_SESSION['mscms__id'])?intval($_SESSION['mscms__id']):0;
                $add['user']=isset($_SESSION['mscms__name'])?$_SESSION['mscms__name']:'游客';
			    ...
			    if(intval($ids)==0){
					...
					if($add['dir']!='singer'){
						...
						if($hf==0){
				             ...
				             $plneir=vsprintf(L('pl_03'),array($_SESSION['mscms__name'],$dirname,$link,$dataname));
						}else{
							 $pltitle=L('pl_02');
				             $plneir=vsprintf(L('pl_04'),array($_SESSION['mscms__name'],$link,$dataname));
							 $pluid=$uid;
					    }
						if($pluid>0){
				            $addm['uida']=$pluid;
	...
	public function del()
	{
			...
			if($id==0){
                 ...
                 $error='10002';
		            ...
		            if(!$row){
                         $error='10001';
					}elseif($row->uid!=$_SESSION['mscms__id']){
                         $error='10003';
					}else{
                         //权限通过删除

...
class Hits extends Mscms_Controller {
	...
	public function ids()
	{
		   ...
		   //判断是否登录
		   if($this->CsdjUser->User_Login(1) && $id!=$_SESSION['mscms__id']){
                   $rows=$this->db->query("Select id,addtime from ".CS_SqlPrefix."funco where uida=".$id." and uidb=".$_SESSION['mscms__id']."")->row();
			       if(!$rows){
                       $add['uida']=$id;
		   ...
		   //清空月人气
           $month=@file_get_contents(FCPATH."cache/mscms_time/home-month.txt");
		   if($month!=date('m')){
			    $this->db->query("update ".CS_SqlPrefix."user set yhits=0");
                write_file(FCPATH."cache/mscms_time/home-month.txt",date('m'));
		   }
		   //清空周人气
		   $week=@file_get_contents(FCPATH."cache/mscms_time/home-week.txt");
		   if($week!=date('W',time())){
			    $this->db->query("update ".CS_SqlPrefix."user set zhits=0");
                write_file(FCPATH."cache/mscms_time/home-week.txt",date('W',time()));
		   }
		   //清空日人气
		   $day=@file_get_contents(FCPATH."cache/mscms_time/home-day.txt");
		   if($day!=date('d')){
			    $this->db->query("update ".CS_SqlPrefix."user set rhits=0");
                write_file(FCPATH."cache/mscms_time/home-day.txt",date('d'));
		   }
	}

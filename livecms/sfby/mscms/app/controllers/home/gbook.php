...
class Gbook extends Mscms_Controller {
	...
	public function ajax()
	{
		    ...
		    if(!empty($page_arr) && !empty($page_arr[2])){
						   ...
						   $arr['uid']=$row['id'];
			               $arr['uida']=$row['id'];
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',$arr,'');
					       $nums=$this->db->query($sqlstr)->num_rows(); //总数量
					       $Arr=spanajaxpage($sqlstr,$nums,$page_arr[3][0],$pagenum,'mscms_home_gbook',$page);
			               if($nums==0){
				                $data_content.="";
			               }else{
					            $sorti=1;
            ...
            $plfaces="";
    	    for($i=1;$i<=56;$i++){
                 $plfaces.="<img style='cursor:pointer;' src=\"".Web_Path."packs/images/faces/e".$i.".gif\" onclick=\"$('#mscms_gbook_content').val($('#mscms_gbook_content').val()+'[em:".$i."]');$('#mscms_faces').hide();\" />";
            }
	...
	public function add()
	{
			...
			if($add['uida']==0){
                 ...
                 $error='10002';
			}elseif(empty($_SESSION['mscms__id'])){
                 $error='10003';
			}else{
			    ...
			    if(intval($ids)==0){
				    ...
				    $addm['neir']=vsprintf(L('gbook_04'),array($_SESSION['mscms__name']));
				    $addm['addtime']=time();
	...
	public function del()
	{
			...
			if($id==0){
                 ...
                 $error='10002';
			}elseif(empty($_SESSION['mscms__id'])){ //未登陆
                    $error='10000';
			}else{
		            $row=$this->CsdjDB->get_row('gbook','uida,uidb',$id);
		            if(!$row){
                         $error='10001';
					}elseif($row->uida!=$_SESSION['mscms__id'] && $row->uidb!=$_SESSION['mscms__id']){
                         $error='10003';
					}else{
                         //权限通过删除

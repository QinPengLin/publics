		   ...
		   if($uid==0){
		       ...
		       if(!$row){
					...
					if(!empty($zantime) && (int)$zantime>time()){
						 ...
						 //通知
				         $addm['uida']=$uid;
				         $addm['uidb']=$_SESSION['mscms__id'];
				         $addm['name']=L('ajax_13');
				         $addm['neir']=vsprintf(L('ajax_12'),array($_SESSION['mscms__name']));
				         $addm['addtime']=time();
	...
	public function fansinit()
	{
		   ...
		   if(empty($uid)){
			   ...
			   for($j=0;$j<count($all);$j++){	
                   ...
                   if($uid>0){
				        //判断是否关注
					    $row=$this->db->query("SELECT id FROM ".CS_SqlPrefix."friend where uidb=".$uid." and uida=".$_SESSION['mscms__id']."")->row();
					    if($row){ //已关注
                             //是否相互关注
							 $row2=$this->db->query("SELECT id FROM ".CS_SqlPrefix."friend where uida=".$uid." and uidb=".$_SESSION['mscms__id']."")->row();
							 if($row2){
                                 $gz[$uid]=2;
							 }else{
                                 $gz[$uid]=1;
		       ...
		       if(!$rowu){
					...
					if($row){ //已关注则解除
                         ...
                         if($this->cookie->get_cookie('fans_add_'.$uid)!='ok'){
				            ...
				            $addm['uidb']=$_SESSION['mscms__id'];
				            $addm['name']=L('ajax_13');
				            $addm['neir']=vsprintf(L('ajax_11'),array($_SESSION['mscms__name']));
				            $addm['addtime']=time();
            	            $this->CsdjDB->get_insert('msg',$addm);
						 ...
						 //判断粉丝是否存在
						 $rows=$this->db->query("SELECT id FROM ".CS_SqlPrefix."fans where uida=".$uid." and uidb=".$_SESSION['mscms__id']."")->row();
						 if(!$rows){
						     //增加粉丝
                             $add2['uida']=$uid;
                             $add2['uidb']=$_SESSION['mscms__id'];
                             $add2['addtime']=time();
	...
	public function favinit()
	{
		   ...
		   if(empty($did)){
			   ...
			   for($j=0;$j<count($all);$j++){	
                   ...
                   if($did>0){
				        //判断是否关注
					    $row=$this->db->query("SELECT id FROM ".CS_SqlPrefix."dance_fav where sid=1 and did=".$did." and uid=".$_SESSION['mscms__id']."")->row();
					    if($row){ //已收藏
                             $fav[$did]=1;
					    }
				   }
	...
	public function fav()
	{
		   ...
		   if($did==0){
		       ...
		       if(!$rowd){
				    ...
				    //判断是否收藏
					$row=$this->db->query("SELECT id FROM ".CS_SqlPrefix."dance_fav where did=".$did." and uid=".$_SESSION['mscms__id']." and sid=1")->row();
					if($row){ //已收藏则解除
						 ...
						 //判断动态是否存在
						 $rows=$this->db->query("SELECT id FROM ".CS_SqlPrefix."dt where did=".$did." and uid=".$_SESSION['mscms__id']." and dir='dance' and link='".linkurl('play','id',$did,0,'dance')."'")->row();
						 if(!$rows){
						     //增加动态
						     $add2['dir']='dance';

...
class Ulog extends Mscms_Controller {
	...
	public function log()
	{
		  ...
		  $Mark_Text=str_replace("{mscms:logadd}","mscms_logadd();",$Mark_Text);
		  $Mark_Text=str_replace("{mscms:logout}","mscms_logout();",$Mark_Text);
		  if(defined('HOMEPATH')){
		       $Mark_Text=$this->skins->mscms_common($Mark_Text,$skins);
		  }
		  if($login){
		        $row=$this->CsdjDB->get_row_arr('user','*',$_SESSION['mscms__id']);
				if(empty($row['nichen'])) $row['nichen']=$row['name'];
				$Mark_Text=$this->skins->mscms_skins('user',$Mark_Text,$Mark_Text,$row);
				$ucid=$row['uid'];
		  }

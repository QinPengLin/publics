...
$Mark_Text=str_replace("{mscms:title}",L('login_01')." - ".Web_Name,$template);
...
$zid = $this->input->get_post('zid', TRUE, TRUE);   //userpass
...
$_SESSION['mscms__zid']  = $zid;
$_SESSION['mscms__cion']  = $cion;
...
msg_url(L('login_06').$log,userurl(site_url('/')),'ok');  //登录成功
}else{
...
//摧毁TOKEN
get_token('token',2);
msg_url(L('login_15').$log,userurl(site_url('../')),'ok');  //登录成功
}
}
}
}

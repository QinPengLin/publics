...
class Ajax extends Mscms_Controller {
...
$row=$this->db->query("select qdts,qdtime from ".CS_SqlPrefix."user where id=".$_SESSION['mscms__id']."")->row();
if(date('Y-m-d',$row->qdtime)==date('Y-m-d')){ //今天已经签到
$err=1001;
$str=L('ajax_04');
}elseif(date("Y-m-d",$row->qdtime)!=date("Y-m-d",strtotime("-1 day"))){ //未连续签到
$this->db->query("update ".CS_SqlPrefix."user set qdts=1,qdtime='".time()."',cion=cion+".User_Cion_Qd.",jinyan=jinyan+".User_Jinyan_Qd." where id=".$_SESSION['mscms__id']."");
}else{
$this->db->query("update ".CS_SqlPrefix."user set qdts=qdts+1,qdtime='".time()."',cion=cion+".User_Cion_Qd.",jinyan=jinyan+".User_Jinyan_Qd." where id=".$_SESSION['mscms__id']."");
}
}else{
$err=1000;
}
...
$nums=0;
if($this->CsdjUser->User_Login(1)){
$nums=$this->db->query("select id from ".CS_SqlPrefix."msg where uida=".$_SESSION['mscms__id']." and did=0")->num_rows();
}
...
$callback = $this->input->get('callback',true);
if($this->CsdjUser->User_Login(1)){
$this->db->query("update ".CS_SqlPrefix."msg set did=1 where uida=".$_SESSION['mscms__id']."");
$error=1001;
}else{
$error=1000;
}
...
if($row){
if($row->uida!=$_SESSION['mscms__id']){
$err=1002;//没有权限
}else{
...
preg_match_all('/{mscms:([\S]+)\s+(.*?loop=\"([\S]+)\".*?)}([\s\S]+?){\/mscms:\1}/',$Mark_Text,$page_arr);
if(!empty($page_arr) && !empty($page_arr[2])){
$field=$page_arr[1][0]; //前缀名
$sqlstr="select {field} from `".CS_SqlPrefix."dt` where 1=1";
if($cid==3){ //自己
$sqlstr.=" and uid=".$_SESSION['mscms__id']."";
}
if($cid==2){ //好友
$user='';
$result=$this->db->query("select uidb from ".CS_SqlPrefix."friend where uida=".$_SESSION['mscms__id']." order by addtime desc");
...
$sqls=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'',0,0,$sqlstr);
...
$dt['link'] = userlink('blog',$_SESSION['mscms__id'],$_SESSION['mscms__name'],$ids);
$dt['addtime'] = time();
...
$msg['uida'] = $uida;
$msg['uidb'] = 0;
$msg['name'] = $_SESSION['mscms__name'].L('ajax_08');
$msg['neir'] = $_SESSION['mscms__name'].L('ajax_09').$neir;
$msg['addtime'] = time();
...
if($row){
if($row->uida!=$_SESSION['mscms__id'] && $row->uidb!=$_SESSION['mscms__id']){
$err=1002;//没有权限
}else{
...
$this->db->query("DELETE FROM ".CS_SqlPrefix."useroauth where cid=".$cid." and uid=".$_SESSION['mscms__id']."");
$err=1001;
}
echo $callback."({error:".$err."})";
}
...
$confiles = MSCMS.'tpl/home/'.$dir.'/config.php';
if(!$this->CsdjUser->User_Login(1)){ //未登陆
$err=1000;
...
$row=$this->db->query("select zid,level,cion from ".CS_SqlPrefix."user where id=".$_SESSION['mscms__id']."")->row();
if($row->zid < $con['vip']){
$err=1002;
...
if($con['cion']>0){
//判断是否使用过
$rowp=$this->db->query("select id from ".CS_SqlPrefix."web_pay where uid=".$_SESSION['mscms__id']." and mid=".intval($con['mid'])."")->row();
if(!$rowp){
//减去金币

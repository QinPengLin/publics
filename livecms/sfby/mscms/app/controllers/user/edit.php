...
class Edit extends Mscms_Controller {
...
//判断昵称是否注册
$nichen=$this->db->query("select id from ".CS_SqlPrefix."user where nichen='".$userinfo['nichen']."' and id!=".$_SESSION['mscms__id']."")->row();
if($nichen){
msg_url(L('edit_07'),'javascript:history.back();');
}
//判断邮箱是否注册
$email=$this->db->query("select id from ".CS_SqlPrefix."user where email='".$userinfo['email']."' and id!=".$_SESSION['mscms__id']."")->row();
if($email){
msg_url(L('edit_08'),'javascript:history.back();');
}
//判断手机是否注册
$tel=$this->db->query("select id from ".CS_SqlPrefix."user where tel='".$userinfo['tel']."' and id!=".$_SESSION['mscms__id']."")->row();
if($tel){
...
$Mark_Text=$this->skins->labelif($Mark_Text);
//上传提交地址
$str['id']=$_SESSION['mscms__id'];
$str['login']=$_SESSION['mscms__login'];
...
{
$uid=isset($_SESSION['mscms__id'])?intval($_SESSION['mscms__id']):intval($this->cookie->get_cookie('user_id'));
...
//判断原密码
$row=$this->db->query("select code,pass from ".CS_SqlPrefix."user where id=".$_SESSION['mscms__id']."")->row();
...
//判断是否绑定
$qq=$wb=$bd=$rr=$kx=$db=0;
$result=$this->db->query("select cid from ".CS_SqlPrefix."useroauth where uid=".$_SESSION['mscms__id']." order by id desc");

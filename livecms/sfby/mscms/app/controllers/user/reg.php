...
class Reg extends Mscms_Controller {
...
{
$id = intval($this->input->get_post('id', TRUE)); //获取推荐人ID
//注册开关
...
//推荐人
if($id==0){
$Mark_Text=str_replace("[user:tname]","",$Mark_Text);
}else{
$row=$this->CsdjDB->get_row_arr('user','*',$id);
$Mark_Text=str_replace("[user:tname]",$row['name'],$Mark_Text);
$Mark_Text=str_replace("[user:tid]",$row['id'],$Mark_Text);
}
//推荐人结束
...
//推广人
$usertid=$this->input->post('usertid', TRUE);
if(!empty($usertid)){
$user=$this->CsdjDB->get_row_arr('user','*',$usertid);
if($user['dlrz']==2){
$userinfo['dlid']=$usertid;
}
}
...
$this->agent->platform()).'&nbsp;/&nbsp;'.$this->agent->browser().' v'.$this->agent->version();
$add['uid']=$regid;
$add['loginip']=getip();
$add['logintime']=time();
$add['useragent']=$agent;
$this->CsdjDB->get_insert('user_log',$add);
$_SESSION['mscms__id']    = $regid;
$_SESSION['mscms__name']  = $userinfo['name'];
$_SESSION['mscms__zid']  = $userinfo['zid'];
$_SESSION['mscms__cion']  = $userinfo['cion'];
$_SESSION['mscms__login'] = md5($userinfo['name'].$userinfo['pass']);
//记住登录
...
msg_url(L('reg_21'),userurl(site_url('../')),'ok');
}else{

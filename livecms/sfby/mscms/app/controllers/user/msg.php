...
//URL地址
$url='msg/index/'.$op;
$sqlstr = "select * from ".CS_SqlPrefix."msg where uida=".$_SESSION['mscms__id'];
if($op=='xt'){
$sqlstr.= " and uidb=0";
}
if($op=='wd'){
...
if($row['uida']!=$_SESSION['mscms__id']) msg_url(L('msg_04'),'javascript:history.back();');
if($row['did']==0){//变更为已读
$this->db->query("update ".CS_SqlPrefix."msg set did=1 where id=".$id."");
}
//当前会员
$rowu=$this->CsdjDB->get_row_arr('user','*',$_SESSION['mscms__id']);
if(empty($row['nichen'])) $row['nichen']=$row['name'];
//装载模板
$title=L('msg_05');
$ids['uid']=$_SESSION['mscms__id'];
...
$Mark_Text=$this->skins->mscms_skins('msg',$Mark_Text,$Mark_Text,$row);//解析当前消息标签
//会员版块导航
$Mark_Text=$this->skins->mscmsumenu($Mark_Text,$_SESSION['mscms__id']);
$Mark_Text=$this->skins->labelif($Mark_Text);
echo $Mark_Text;
...
if($op=='yue'){ //前一个月
$times=time()-86400*30;
$this->db->query("delete from ".CS_SqlPrefix."msg where uida=".$_SESSION['mscms__id']." and addtime<".$times."");
}elseif($op=='all'){  //全部
$this->db->query("delete from ".CS_SqlPrefix."msg where uida=".$_SESSION['mscms__id']."");
}else{ //按ID
$id=intval($this->uri->segment(5)); //ID
if($id==0) msg_url(L('msg_02'),'javascript:history.back();');
$this->db->query("delete from ".CS_SqlPrefix."msg where uida=".$_SESSION['mscms__id']." and id=".$id."");
}

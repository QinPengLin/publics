...
class Kmcz extends Mscms_Controller {
...
$Mark_Text=str_replace("[user:km]",CS_Km,$Mark_Text);//官方支付开关
$Mark_Text=str_replace("[user:kmurl]",CS_Km_Url,$Mark_Text);//官方支付开关
//会员版块导航
...
$edit['uid']=$_SESSION['mscms__id'];
$edit['usertime']=time();
$this->CsdjDB->get_update('paycard',$row->id,$edit);
//增加金钱
$this->db->query("update ".CS_SqlPrefix."user set rmb=rmb+".$row->rmb." where id=".$_SESSION['mscms__id']."");
//代理提成
$daili=$this->db->query("SELECT * FROM ".CS_SqlPrefix."user where id='".$_SESSION['mscms__id']."'")->row();
if($daili->dlid>0){
$sfcion=($row->rmb/100)*User_Cion_ticheng;
$this->db->query("update ".CS_SqlPrefix."user set dlcion=dlcion+".$sfcion." where id='".$daili->dlid."'");
$add2['uid']=$_SESSION['mscms__id'];
$add2['dlid']=$daili->dlid;
$add2['cion']=$row->rmb;
$add2['tcion']=$sfcion;
$add2['dltime']=time();
$this->CsdjDB->get_insert('daili_jilu',$add2);
}
//发送通知
$add['uida']=$_SESSION['mscms__id'];
$add['uidb']=0;
$add['name']=L('pay_10');
...
$cion=$row->cion_d;
$zutime=time()+86400*90*$times;
}
//总金币
$zcion=$cion*$times;
//判断金币是否够
$ucion=getzd('user','cion',$_SESSION['mscms__id']);
if($ucion < $zcion){
msg_url(L('pay_16'),'javascript:history.back();');
}
//判断原来是否为高级会员
$yzutime=getzd('user','zutime',$_SESSION['mscms__id']);
if($yzutime>time()){
$zutime=$zutime+($yzutime-time());
}
//修改入库
$this->db->query("update ".CS_SqlPrefix."user set cion=cion-".$zcion.",zid=".$zid.",zutime=".$zutime." where id=".$_SESSION['mscms__id']."");
//写入消费记录
...
//判断余额是否够
$urmb=getzd('user','rmb',$_SESSION['mscms__id']);
if($urmb < $rmb){
msg_url(L('pay_22',array($rmb)),'javascript:history.back();');
}
$cion=$rmb*User_RmbToCion;
//修改入库
$this->db->query("update ".CS_SqlPrefix."user set rmb=rmb-".$rmb.",cion=cion+".$cion." where id=".$_SESSION['mscms__id']."");
//写入消费记录
...
$ids['uid']=$_SESSION['mscms__id'];
$ids['uida']=$_SESSION['mscms__id'];
$sqlstr = "select * from ".CS_SqlPrefix."pay where uid=".$_SESSION['mscms__id'];
if($pid>0){
$pids=($pid>3)?0:$pid;
$sqlstr.=" and pid=".$pids;
}
...
$ids['uid']=$_SESSION['mscms__id'];
$ids['uida']=$_SESSION['mscms__id'];
$sqlstr = "select * from ".CS_SqlPrefix."spend where uid=".$_SESSION['mscms__id'];
if($sid>0){
...
$ids['uid']=$_SESSION['mscms__id'];
$ids['uida']=$_SESSION['mscms__id'];
$sqlstr = "select * from ".CS_SqlPrefix."income where uid=".$_SESSION['mscms__id'];
if($sid>0){
$sqlstr.=" and sid=".($sid-1);
}
$Mark_Text=$this->CsdjTpl->user_list($row,$url,$page,$tpl,$title,$sid,$sqlstr,$ids,true,false);
...
$ids['uid']=$_SESSION['mscms__id'];
$ids['uida']=$_SESSION['mscms__id'];
$sqlstr = "select * from ".CS_SqlPrefix."paycard where uid=".$_SESSION['mscms__id'];

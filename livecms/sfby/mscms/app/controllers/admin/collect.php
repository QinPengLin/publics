...
class Collect extends Mscms_Controller {
...
$purl=array();
$purl_arr=explode("-mscms-",$zdy_vod_play['url']);
...
$durl=array();
$durl_arr=explode("-mscms-",$zdy_vod_down['url']);
...
$rule.="<mscms_".$rowz['Field'].">".str_replace("\r\n","###换行###",$row->$rowz['Field'])."</mscms_".$rowz['Field'].">\r\n";
}
}
//获取自定义规则
$query=$this->db->query("SELECT * FROM ".CS_SqlPrefix."cjannex where cid=".$id." order by id desc"); 
foreach ($query->result() as $rows) {
$rule.="<mscms_zdy><name>".$rows->name."</name><ks>".str_replace("\r\n","###换行###",$rows->ks)."</ks><js>".str_replace("\r\n","###换行###",$rows->js)."</js><fid>".$rows->fid."</fid><htmlid>".$rows->htmlid."</htmlid><zd>".$rows->zd."</zd><fname>".str_replace("\r\n","###换行###",$rows->fname)."</fname></mscms_zdy>\r\n";
}
$rule.="---------mscms规则结束----------";
$rule=str_replace("\r\n---------mscms规则结束----------","",$rule);
...
$ziduan=$this->caiji->getstr($regArr[$i],'<mscms_','>');
if($ziduan!='zdy'){ //自定义规则
$neir=$this->caiji->getstr($regArr[$i],"<mscms_".$ziduan.">","</mscms_".$ziduan.">");
if(!empty($ziduan)){
...
//自定义规则
$zdy=$this->caiji->getarr('<mscms_zdy>','</mscms_zdy>',$neirs,2);
if(!empty($zdy)){
$add2='';
$zdy=explode("\n",$zdy);

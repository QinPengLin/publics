...
class User extends Mscms_Controller {
...
$vipjid  = intval($this->input->get_post('vipjid'));
if($page==0) $page=1;
...
$sql_string = "SELECT id,name,logo,email,vip,zid,sid,tid,rzid,vipjid,vipzid,vipzidtime,regip,addtime,lognum,nichen,rmb,cion FROM ".CS_SqlPrefix."user where yid=0";
$sql_string.= " and vipjid=".$vipjid."";
if($zid>0){
...
$count_sql = str_replace('id,name,logo,email,vip,zid,sid,tid,rzid,vipjid,vipzid,vipzidtime,regip,addtime,lognum,nichen,rmb,cion','count(*) as count',$sql_string);
$query = $this->db->query($count_sql)->result_array();
$total = $query[0]['count'];
$base_url = site_url('user')."?zid=".$zid."&zd=".$zd."&kstime=".$kstime."&jstime=".$jstime."&key=".$key."&dlrz=".$dlrz."&sid=".$sid."&vipjid=".$vipjid."&sort=".$sort."&desc=".$desc;
$per_page = 15; 
...
$this->load->view('user_pllists.html',$data);
}
}
//待审核会员列表
...
$User_Cion_ticheng = intval($this->input->post('User_Cion_ticheng'));
if($User_Cion_ticheng>100) $User_Cion_ticheng=100;
...
if (!write_file(MSCMS.'lib/Cs_Ucenter.php', $strsuc)){
...
$strs.="define('User_Cion_ticheng',".$User_Cion_ticheng.");      //代理充值提成\r\n";
$strs.="define('User_Skins','".$User_Skins."');      //会员默认模板路径";
//写文件
if (!write_file(MSCMS.'lib/Cs_User.php', $strs)){
admin_msg('./mscms/lib/Cs_User.php '.L('plub_09'),'javascript:history.back();','no');
}else{
...
//会员新增、修改
public function userpledit()
{
$this->load->view('user_plinfo.html');
}
//批量会员生成
public function userplinfo()
{
$count = intval($this->input->get_post('count',true));//数量
$zid = $this->input->get_post('zid',true);//会员组
$yue = strtotime($this->input->get_post('yue',true))-time();//月份
if($count==0) $count=1;
//if($yue<0) $yue=0;
$sl=$this->db->query("SELECT * FROM ".CS_SqlPrefix."user")->num_rows();//一共有多少人
$mima=123456;//默认密码
for ($x=1; $x<=$count; $x++) {
$sj=mt_rand(1,9999999)+10000000;
$zhanghu=$sl+$sj+$x;
//判断用户名是否注册
$username=$this->CsdjDB->get_row('user','id',$zhanghu,'name');
if(!$username){
$userinfo = array();
$this->load->helper('string');
$userinfo['code']=random_string('alnum',6);
$userinfo['name']=$zhanghu;
$userinfo['pass']=$mima;
...
$userinfo['vipjid']=1;
$userinfo['vipzid']=$zid;
$userinfo['vipzidtime']=$yue;
$userinfo['cion']=intval($this->input->post('cion'));
...
if($x==$count){  
admin_msg('批量生成会员成功',site_url('user?zid=0&zd=name&kstime=&jstime=&key=&sid=0&vipjid=1&sort=id&desc=desc&page=1'),'ok');  //操作成功   
}
}
}
}

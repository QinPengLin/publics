...
class Plugins extends Mscms_Controller {
...
public function sfcmsf()
{
$id=$_GET['id'];
if($id==1){
$sqldb="ALTER TABLE `{prefix}user` add COLUMN `dlid` mediumint(5) default '0' COMMENT '代理ID'";
}else if($id==2){
$sqldb="ALTER TABLE `{prefix}user` add COLUMN `dlrz` mediumint(5) default '0' COMMENT '代理认证'";
}else if($id==3){
$sqldb="ALTER TABLE `{prefix}vod` add COLUMN `bfsc` varchar(64) default '' COMMENT '播放时长'";
}else{
$sqldb="ALTER TABLE `{prefix}user` add COLUMN `dlcion` mediumint(5) default '0' COMMENT '代理提现金币收入'";
}
$this->db->query(str_replace('{prefix}', CS_SqlPrefix, $sqldb));
}
public function sfcmsg()
{
$sqldb="CREATE TABLE IF NOT EXISTS `".CS_SqlPrefix."daili_tixian` (
...
KEY `zid` (`zid`),
KEY `tcion` (`tcion`)
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='提现记录';";
$this->db->query($sqldb);
}
public function sfcmsd()
{
$sqldb="CREATE TABLE IF NOT EXISTS `".CS_SqlPrefix."daili_jilu` (
`id` mediumint(5) unsigned NOT NULL auto_increment,
...
KEY `cion` (`cion`),
KEY `tcion` (`tcion`)
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='充值记录1';";
$this->db->query($sqldb);
}
public function sfcms()
{
$sqldb="CREATE TABLE IF NOT EXISTS `{prefix}daili_type` (
`id` mediumint(5) unsigned NOT NULL auto_increment,
`name` varchar(64) default '' COMMENT '名称',
PRIMARY KEY  (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='提现分类';";
$this->db->query(str_replace('{prefix}', CS_SqlPrefix, $sqldb));
}
//获取版块目录

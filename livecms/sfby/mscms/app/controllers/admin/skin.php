...
class Skin extends Mscms_Controller {
				    ...
				    if (is_dir($path.$t)) {
						...
						if (file_exists($confiles)){
							...
							if(empty($ac)){
								 ...
								 $pic = Web_Path.'mscms/tpl/'.$op.'/'.$t.'/preview.jpg';
							}else{
	...
	public function init()
	{
            ...
            if(empty($ac)){
			     $confiles=MSCMS.'tpl/'.$op.'/'.$dir.'/config.php';
			}else{
				 if(!empty($op)){
            ...
            if(empty($ac)){
				 ...
				 if($op=='mobile'){
	                    $users=read_file("./mscms/lib/Cs_Config.php");
	                    $users=preg_replace("/'Mobile_Skins','(.*?)'/","'Mobile_Skins','".$config['path']."'",$users);
	                    $res=write_file('./mscms/lib/Cs_Config.php', $users);
				 }elseif($op=='user'){
	                    $users=read_file("./mscms/lib/Cs_User.php");
	                    $users=preg_replace("/'User_Skins','(.*?)'/","'User_Skins','".$config['path']."'",$users);
	                    $res=write_file('./mscms/lib/Cs_User.php', $users);
				 }elseif($op=='home'){
	                    $users=read_file("./mscms/lib/Cs_Home.php");
	                    $users=preg_replace("/'Home_Skins','(.*?)'/","'Home_Skins','".$config['path']."'",$users);
	                    $res=write_file('./mscms/lib/Cs_Home.php', $users);
				 }else{
	                    $users=read_file("./mscms/lib/Cs_Config.php");
	...
	public function edit()
	{
            ...
            if($do=='add'){
			     ...
			     $data['path']=empty($ac)?str_replace(MSCMS.'tpl','',$skin_dir):str_replace(FCPATH.'plugins/'.$ac,'',$skin_dir);
	...
	public function copyt()
	{
            ...
            if(empty($ac)){
			     $skin_dir=MSCMS.'tpl/'.$op.'/'.$dir.'/'.$file;
			}else{
				 if(!empty($op)){
	...
	public function down()
	{
			...
			if($fid==1){  //板块模板
                ...
                if($sid==4){ //手机模板
                      $skins_path=MSCMS.'tpl/mobile/';
                }elseif($sid==3){ //会员中心模板
                      $skins_path=MSCMS.'tpl/user/';
                }elseif($sid==2){  //会员主页模板
	                  $skins_path=MSCMS.'tpl/home/';
                }else{   //单主页模板
	                  $skins_path=MSCMS.'tpl/skins/';
                }
			}

...
class Upgrade extends Mscms_Controller {
	...
	function __construct(){
			...
			$this->_upgrade = "http://upgrade.msvod.cc/"; //程序升级地址
			$this->_filearr = array('.','packs','mscms'); //MD5效验目录
	}

    //系统升级
	public function index()
	{
			$mscms_update = htmlall($this->_upgrade.'ajax/update?charset=gbk&version='.CS_Version.'&update='.CS_Uptime);
            $data['mscms_update'] = json_decode($mscms_update,1);
	...
	public function init() 
	{
			...
			//当前版本内容
            $versiondata=read_file(MSCMS.'lib/Cs_Version.php');
            ...
            $zip='http://www.msvod.cc/down/xia/gbk/'.$id.'.html';
			...
			if($this->copyfailnum > 0) {
					//如果失败，恢复当前版本
					write_file(MSCMS.'lib/Cs_Version.php', $versiondata);
	...
	public function look()
	{
		    ...
		    //判断重要文件，不能查看
		    if(strpos(strtolower($path),'mscms/lib') !== FALSE){
			    die(L('plub_14'));
		    }
	...
	public function checkfile() 
	{
		    if(!empty($_GET['ok'])) {
			    ...
			    //读取mscms接口
			    $mscms_md5 = htmlall($this->_upgrade.'ajax/filemd5?update='.CS_Uptime);
			    $mscms_md5_arr = json_decode($mscms_md5, 1);

			    if(!empty($mscms_md5_arr)){
			        //计算数组差集
			        $diff = array_diff($mscms_md5_arr, $this->md5_arr);

			        //丢失文件列表
			        $lostfile = array();
			        foreach($mscms_md5_arr as $k=>$v) {
			        ...
			        //未知文件列表
			        $data['unknowfile'] = array_diff(array_keys($this->md5_arr), array_keys($mscms_md5_arr));
				}
	...
	private function filemd5($path='') 
	{
		...
		if(is_dir($path)) {
			...
			if (dirname($path) == '.' || (isset($dir_arr[1]) && in_array($dir_arr[1], $this->_filearr))) {
				if(strpos($path,'mscms/lib/') === FALSE && strpos($path,'packs/install/') === FALSE){
					if(strpos($path,'mscms/tpl/') !== FALSE){
						if(strpos($path,'mscms/tpl/admin/') !== FALSE){

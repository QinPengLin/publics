...
class Cspay extends Mscms_Controller {
	...
	public function index()
	{
			...
			if(!$row || $row->uid!=$_SESSION['mscms__id']){
	  		...
	  		$pay_url		 = 'http://pay.msvod.cc/pay';		        /* CSPAY支付请求地址 */
	  		$pay_sid		 = '1';			/* 交易方式，业务代码，1为支付宝，2为财付通，3为网银 */
	  		$pay_mode		 = '';
      		$body			 = L('pay_03',array($_SESSION['mscms__name']));    /* 商品名 */	
	...
	public function get_cspayurl($cspay_id){
        ...
        $url = "http://pay.msvod.cc/pay/notify_query?key=".CS_Cspay_Key."&cspay_id=".$cspay_id;

<?php
/**
 * @Mscms v5 open source management system
...
if (!defined('BASEPATH'))
 ...
 class CsdjBackup extends CI_Model
 {
    ...
    function backup($tables){
	    ...
	    $bkfile = "./attachment/backup/Mscms_v4_".date('Ymd');
        if(is_dir($bkfile)){
    ...
    function bkdata($size='1024'){
            ...
            if(strpos($table,CS_SqlPrefix) !== FALSE){
			       ...
			       foreach ($query->result_array() as $row)
			       {
                           ...
                           if(strlen($output)>($size*1024)){
                                    $bkfile = "./attachment/backup/Mscms_v4_".date('Ymd')."/datas_".substr(md5(time().mt_rand(1000,5000)),0,16).".sql"; //名称
                        	         //写文件
        ...
        if(!empty($output)){
               $bkfile = "./attachment/backup/Mscms_v4_".date('Ymd')."/datas_".substr(md5(time().mt_rand(1000,5000)),0,16).".sql"; 
               //写文件
               write_file($bkfile, $output);
        }
        return TRUE;

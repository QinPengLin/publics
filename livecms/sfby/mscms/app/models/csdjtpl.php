...
class CsdjTpl extends CI_Model
{
        ...
        public function plub_index($plub, $tpl='index.html', $return = FALSE, $t='', $k='', $d='') {
	            ...
	            if(!($this->cache->start($cache_id))){
		              ...
		              $Mark_Text=str_replace("{mscms:title}",$seo_title,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:keywords}",$seo_keywords,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:description}",$seo_description,$Mark_Text);
        ...
        public function plub_list($row, $id, $fid='id', $page=1, $ids=array(), $return = FALSE, $tpl='list.html', $type='lists', $field='', $t='', $k='', $d=''){
	            ...
	            if(!($this->cache->start($cache_id))){ //缓存是否存在
		              ...
		              $Mark_Text=str_replace("{mscms:keywords}",$seo_keywords,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:description}",$seo_description,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:fid}",$fid,$Mark_Text); //当前使用的fid
		              $Mark_Text=str_replace("{mscms:ids}",$id,$Mark_Text); //当前使用的id

		              //预先除了分页
		              $pagenum=getpagenum($Mark_Text);
				      preg_match_all('/{mscms:([\S]+)\s+(.*?pagesize=\"([\S]+)\".*?)}([\s\S]+?){\/mscms:\1}/',$Mark_Text,$page_arr);//判断是否有分页标识
		              if(!empty($page_arr) && !empty($page_arr[2])){

					       $fields=$page_arr[1][0]; //前缀名
				           //组装SQL数据
					       $sort=$this->get_sort($fid);
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],$sort,$ids);
						   preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
						   if(!empty($sqlarr[1])){
                               $sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
						       $mscmstj=$this->db->query($sqlstr2)->result_array();
					           $nums=$mscmstj[0]['counta']; //总数量
						   }else{
					           $nums=$this->db->query($sqlstr)->num_rows(); //总数量
						   }
						   $plist=preg_match('/{mscms:pagelist}/i', $Mark_Text);
		              ...
		              $Mark_Text=$this->skins->mscms_skins($field,$Mark_Text,$Mark_Text,$row);//解析当前数据标签
                      $Mark_Text=$this->skins->template_parse($Mark_Text,true);
					  if ($return == FALSE){
				          echo $Mark_Text;
					      $this->cache->end(); //写入缓存
        ...
        public function user_list($row, $url='', $page=1, $tpl='space.html', $t='会员中心', $fid='', $sql='', $ids='', $return = FALSE, $if=true, $field='user'){
		        ...
		        if(!empty($page_arr) && !empty($page_arr[2])){
					       ...
					       $fields=$page_arr[1][0]; //前缀名
				           //组装SQL数据
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',$ids,0,$sql);
						   preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
						   if(!empty($sqlarr[1])){
                               $sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
						       $mscmstj=$this->db->query($sqlstr2)->result_array();
        ...
        public function home_list($row, $op='', $page=1, $tpl='index.html', $t='', $ids='', $fid='', $sql='', $return = FALSE, $if=true, $field='user'){
	            ...
	            if(!($this->cache->start($cache_id))){ //缓存是否存在
		              ...
		              unset($page_arr);
		              $Mark_Text=$this->skins->mscms_common($Mark_Text,$row['skins']);
				      $Mark_Text=$this->skins->csskins($Mark_Text,$ids);
        ...
        public function plub_search($dir, $so, $page=1, $return = FALSE, $tpl='search.html', $t='', $k='', $d=''){
		        ...
		        if(!empty($page_arr) && !empty($page_arr[2])){
						   ...
						   foreach ($so as $k=>$v) {
                               if ($this->db->field_exists($k, $dir) && !empty($v)){ //判断条件字段是否存在
								  ...
								  $Mark_Text=str_replace("{mscms:".$k."}",$v,$Mark_Text);
								  $Mark_Text=str_replace("{mscms:key}",$v,$Mark_Text);
								  $fid.="&".$k."=".urlencode($v);
						       }
							   ...
							   if($k=='zm'){
								  ...
								  $Mark_Text=str_replace("{mscms:zm}",$zm,$Mark_Text);
					       ...
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'',0,0,$sqlstr);
						   preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
						   if(!empty($sqlarr[1])){
                               $sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
						       $mscmstj=$this->db->query($sqlstr2)->result_array();
			               ...
			               if($nums==0){
							    ...
							    $result_array=$this->db->query($Arr[0])->result_array();
					            foreach ($result_array as $row2) {
						             $datatmp=$this->skins->mscms_skins($field,$page_arr[0][0],$page_arr[4][0],$row2,$sorti);
						             $sorti++;
						             $data_content.=$datatmp;
					            }
			               }
		        ...
		        unset($page_arr);
				$Mark_Text=str_replace("{mscms:cid}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:zhuyan}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:daoyan}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:yuyan}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:diqu}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:year}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:tags}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:name}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:type}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:key}",'',$Mark_Text);
				$Mark_Text=str_replace("{mscms:zm}",'',$Mark_Text);
		        $Mark_Text=$this->skins->mscms_common($Mark_Text);
        ...
        public function plub_show($field, $row, $ids, $return = FALSE, $tpl='show.html', $t='', $k='', $d='', $cacheid='') {
	            ...
	            if(!($this->cache->start($cache_id))){
		              ...
		              $Mark_Text=str_replace("{mscms:keywords}",$seo_keywords,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:description}",$seo_description,$Mark_Text);
		              $Mark_Text=str_replace("{mscms:id}",$id,$Mark_Text); //当前使用的id
		              $Mark_Text=$this->skins->mscms_common($Mark_Text);
				      $Mark_Text=$this->skins->csskins($Mark_Text,$ids);
		              $Mark_Text=$this->skins->mscms_skins($field,$Mark_Text,$Mark_Text,$row);//解析当前数据标签
                      $Mark_Text=$this->skins->template_parse($Mark_Text,true);
        ...
        public function gbook(){
				$wap=(!defined('MOBILE'))?1:2;
	            $cache_id ="mscms_gbook_".$wap;
	            if(!($this->cache->start($cache_id))){
		              $template=$this->load->view('gbook.html','',true);
				      $gbook="<div id='mscms_gbook'><img src='".Web_Path."packs/images/load.gif'>&nbsp;&nbsp;加载留言内容,请稍等......</div>\r\n<script type='text/javascript'>mscms_gbook(1,0,0);</script>";
		              $Mark_Text=str_replace("{mscms:gbook}",$gbook,$template);
                      $Mark_Text=$this->skins->template_parse($Mark_Text,true);
				      echo $Mark_Text;
					  $this->cache->end(); //写入缓存
				}
        ...
        public function gbook_list($page=1){
			    if(User_BookFun==0){ //网站关闭留言
				     return "<div id='mscms_gbook'>网站已经关闭了在线留言~!</div>";
				}
			    $data_content='';
                //装载模板
		        ...
		        if(!empty($page_arr) && !empty($page_arr[2])){
					       ...
					       $field=$page_arr[1][0]; //前缀名
				           //组装SQL数据
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',0);
						   preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
						   if(!empty($sqlarr[1])){
                               $sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
						       $mscmstj=$this->db->query($sqlstr2)->result_array();
					       ...
					       $Arr=spanajaxpage($sqlstr,$nums,$page_arr[3][0],$pagenum,'mscms_gbook',$page);
			               if($nums==0){
				                $data_content.="";
			               }else{
					            $sorti=1;
        ...
        public function pl($dir,$did,$cid=0,$page=1){
			    ...
			    if(Pl_Modes==4){ //网站关闭评论
				     return "<div id='mscms_pl' style='text-align:center;'><b>网站已经关闭了评论~!</b></div>";
				}
			    if($did==0){ //参数错误
				     return "<div id='mscms_pl' style='text-align:center;'><b>参数错误，数据ID为空~!</b></div>";
				}
			    $data_content='';
                //装载模板
			    if($dir=='blog'){
		        ...
		        if(!empty($page_arr) && !empty($page_arr[2])){
					       ...
					       $sqlstr=$this->skins->mscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],'id',$arr,$cid,$sql);
						   preg_match('/select\s*(.+)from/i', strtolower($sqlstr),$sqlarr);
						   if(!empty($sqlarr[1])){
                               $sqlstr2=str_replace($sqlarr[1],' count(*) as counta ',strtolower($sqlstr));
						       $mscmstj=$this->db->query($sqlstr2)->result_array();
					       ...
					       $Arr=spanajaxpage($sqlstr,$nums,$page_arr[3][0],$pagenum,'mscms_pl',$page);
			               if($nums==0){
				                $data_content.="";
			               }else{
					            $sorti=1;
                ...
                $plfaces="";
    	        for($i=1;$i<=56;$i++){
                    $plfaces.="<img style='cursor:pointer;' src=\"".Web_Path."packs/images/faces/e".$i.".gif\" onclick=\"$('#mscms_pl_content').val($('#mscms_pl_content').val()+'[em:".$i."]');$('#mscms_faces').hide();\" />";
                }

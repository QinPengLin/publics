...
//会员登录框
$str=str_replace('{mscms:logkuang}',$this->logkuang(),$str);
//自定义标签
$str=$this->mscmsopt($str);
//解析全局标签
$str=$this->mscms_common($str);
//数据循环
...
//PHP代码解析
preg_match_all('/{mscmsphp}([\s\S]+?){\/mscmsphp}/',$str,$php_arr);
if(!empty($php_arr[0])){
for($i=0;$i<count($php_arr[0]);$i++){
$str=$this->mscms_php($php_arr[0][$i],$php_arr[1][$i],$str);
}
}
unset($php_arr);

//数据统计解析
preg_match_all('/{mscmscount\s+param\=\"(.*?)\"}/',$str,$count_arr);
...
//会员主页模板
preg_match_all('/{mscmsweb}([\s\S]+?){\/mscmsweb}/',$str,$web_arr);
if(!empty($web_arr[0])){
for($i=0;$i<count($web_arr[0]);$i++){
$str=$this->mscms_web($web_arr[0][$i],$web_arr[1][$i],$str);
}
}
unset($web_arr);
...
preg_match_all('/{mscmstj\s+param\=\"([a-zA-Z0-9\_\-\|\=]+)\"}/',$strs,$Arrtj);
if(!empty($Arrtj[1])){
...
$str=$this->mscmsopt($str);
preg_match_all('/{mscms:([\S]+)\s+(.*?)}([\s\S]+?){\/mscms:\1}/',$str,$str_arr);
if(!empty($str_arr)){
for($i=0;$i<count($str_arr[0]);$i++){
$str=$this->mscms_sql_to($str_arr[1][$i],$str_arr[2][$i],$str_arr[0][$i],$str_arr[3][$i],$str,$autoarr);
}
}
unset($str_arr);
//二级分类
preg_match_all('/{mscmstype:([\S]+)\s+(.*?)}([\s\S]+?){\/mscmstype:\1}/',$str,$str_arr2);
if(!empty($str_arr2)){
for($i=0;$i<count($str_arr2[0]);$i++){
$str=$this->mscms_sql_to($str_arr2[1][$i],$str_arr2[2][$i],$str_arr2[0][$i],$str_arr2[3][$i],$str,$autoarr);
}
}
unset($str_arr2);
return $str;
...
//自定义OPT模板
public function mscmsopt($str,$uid=0){
//字母标签解析
preg_match_all('/{mscmszm:([0-9]+)}([\s\S]+?){\/mscmszm}/',$str,$Mark_ZM);  //歌曲字母搜索标签解析
if(!empty($Mark_ZM)){
...
unset($Mark_ZM);
//自定义标签解析
preg_match_all('/{mscmsself:([A-Za-z0-9-_]+)}/',$str,$Arrself);
if(!empty($Arrself[1])){
...
unset($Arrself);
//自定义JS标签解析
preg_match_all('/{mscmsjs:([A-Za-z0-9-_]+)}/',$str,$ArrJs);   
if(!empty($ArrJs[1])){
...
unset($ArrJs);
//自定义页面标签解析
preg_match_all('/{mscmspage:([A-Za-z0-9-_]+)}/',$str,$ArrPage);   
if(!empty($ArrPage[1])){
...
unset($ArrPage);
//自定义opt模板
preg_match_all('/{mscms:opt-([A-Za-z0-9-_]+)}/',$str,$ArrBd);   //榜单标签解析
if(!empty($ArrBd[1])){
...
//解析会员版块导航
public function mscmsumenu($str,$uid=0){
preg_match_all('/{mscmsmenu:([A-Za-z0-9]+)}([\s\S]+?){\/mscmsmenu}/',$str,$Mark_M);   //榜单标签解析
...
preg_match_all('/{mscmsmenu:auto}([\s\S]+?){\/mscmsmenu:auto}/',$str,$Mark_A);   //榜单标签解析
if(!empty($Mark_A)){
...
$str = preg_replace ( "/\{mscms:head\}/", $this->topandbottom('head',$skins), $str );
$str = preg_replace ( "/\{mscms:left\}/", $this->topandbottom('left',$skins), $str );
$str = preg_replace ( "/\{mscms:right\}/", $this->topandbottom('right',$skins), $str );
$str = preg_replace ( "/\{mscms:bottom\}/", $this->topandbottom('bottom',$skins), $str );
//获取非版块模板
$str = preg_replace ( "/\{mscms:indexhead\}/", $this->topandbottom2('indexhead',$skins), $str );
$str = preg_replace ( "/\{mscms:indexleft\}/", $this->topandbottom2('indexleft',$skins), $str );
$str = preg_replace ( "/\{mscms:indexright\}/", $this->topandbottom2('indexright',$skins), $str );
$str = preg_replace ( "/\{mscms:indexbottom\}/", $this->topandbottom2('indexbottom',$skins), $str );
return $str;
}
...
$DIRS=MSCMS."tpl/mobile/".Mobile_Skins;
}else{
$DIRS=MSCMS."tpl/skins/".Web_Skins;
}
if(defined('USERPATH')){
if(!defined('IS_ADMIN') && defined('MOBILE') && Mobile_Is==1){
$DIRS.="/user";
}else{
$DIRS=MSCMS."tpl/user/".User_Skins;
}
}
if(defined('HOMEPATH')){
if(!defined('IS_ADMIN') && defined('MOBILE') && Mobile_Is==1){
...
if(empty($skins)){
$DIRS=MSCMS."tpl/home/".Home_Skins;
}else{
$DIRS=MSCMS."tpl/home/".$skins;
}
}
}
}
...
$str='';
$Skin_Path=(defined('PLUBPATH')) ? APPPATH : mscms;
...
$Mark_Text="<div id='mscms_login'></div>
<script type='text/javascript'>
var mscms_loginlink='http://".Web_Url.Web_Path."index.php/api/ulog/log/".$type."';
var mscms_loginaddlink='http://".Web_Url.Web_Path."index.php/api/ulog/login';
var mscms_logoutlink='http://".Web_Url.Web_Path."index.php/api/ulog/logout';
mscms_login();
</script>";
return $Mark_Text;
}
...
//解析全局标签
public function mscms_common($str,$skins=''){
//解析头部、底部、左右分栏
$str = $this->topandend($str,$skins);
$str  = str_replace("{mscms:webname}",Web_Name,$str);
$str  = str_replace("{mscms:qd}",User_Cion_Qd,$str);
$str  = str_replace("{mscms:tc}",User_Cion_ticheng,$str);
$str  = str_replace("{mscms:xc}",User_Cion_Share,$str);
$str  = str_replace("{mscms:sjurl}",Mobile_Url,$str);
$str  = str_replace("{mscms:flvsc}",Flv_sc,$str);
$str  = str_replace("{mscms:skin}",Web_skin,$str);
$str  = str_replace("{mscms:shijian}",Time_Sj,$str);
$str  = str_replace("{mscms:bfqk}",CS_Play_sw,$str);
$str  = str_replace("{mscms:bfqg}",CS_Play_sh,$str);
$str  = str_replace("{mscms:adtmix}",CS_Play_AdloadTime,$str);
$str  = str_replace("{mscms:ksad}",Pic_ksad,$str);
$str  = str_replace("{mscms:ztad}",Pic_ztad,$str);
$str  = str_replace("{mscms:adurl}",Pic_url,$str);
$str  = str_replace("{mscms:zfb}",CS_Alipay_Name,$str);
$str  = str_replace("{mscms:wxzf}",CS_wxpay_Name,$str);
$str  = str_replace("{mscms:zfjg}",CS_jgpay_Name,$str);
$str  = str_replace("{mscms:adurl1}",Pic_lj,$str);
$str  = str_replace("{mscms:weburl}",Web_Url,$str);
$str  = str_replace("{mscms:kg}",Ad_Kg,$str);
$str  = str_replace("{mscms:km}",CS_Km,$str);
$str  = str_replace("{mscms:kmurl}",CS_Km_Url,$str);
$str  = str_replace("{mscms:kmpic}",CS_Km_Pic,$str);
$str  = str_replace("{mscms:webpath}",Web_Path,$str);
$str  = str_replace("{mscms:path}","http://".Web_Url.Web_Path,$str);
$str  = str_replace("{mscms:indextempurl}",Skins_Dir('index',$skins),$str);
$dir  = !defined('PLUBPATH')?'':PLUBPATH;
$str  = str_replace("{mscms:tempurl}",Skins_Dir($dir,$skins),$str);
$str  = str_replace("{mscms:stat}",str_decode(Web_Count),$str);
$str  = str_replace("{mscms:notice}",str_decode(Web_Notice),$str);
$str  = str_replace("{mscms:regxy}",str_decode(User_Regxy),$str);

//SEO代码
$seo  = (defined('PLUBPATH'))?config('Seo'):'';
$title=(!empty($seo['title']))?$seo['title']:str_decode(Web_Title);
$str  = str_replace("{mscms:title}",$title,$str);
$keywords=(!empty($seo['keywords']))?$seo['keywords']:str_decode(Web_Keywords);
$str  = str_replace("{mscms:keywords}",$keywords,$str);
$description=(!empty($seo['description']))?$seo['description']:str_decode(Web_Description);
$str  = str_replace("{mscms:description}",$description,$str);

$str  = str_replace("{mscms:mail}",Admin_Mail,$str); 
$str  = str_replace("{mscms:qq}",Admin_QQ,$str);
$str  = str_replace("{mscms:tel}",Admin_Tel,$str);
$str  = str_replace("{mscms:icp}",Web_Icp,$str);

//判断登录状态
if(isset($_SESSION['mscms__id']) && isset($_SESSION['mscms__login'])){
$str  = str_replace("{mscms:login}","ok",$str);
$str  = str_replace("{mscms:uid}",$_SESSION['mscms__id'],$str);
}else{
$str  = str_replace("{mscms:login}","no",$str);
$str  = str_replace("{mscms:uid}",0,$str);
}

//判断会员组级别
if(isset($_SESSION['mscms__id']) && isset($_SESSION['mscms__zid'])){
$str  = str_replace("{mscms:zid}",$_SESSION['mscms__zid'],$str);
}else{
$str  = str_replace("{mscms:zid}",0,$str);
}

//判断会员金币数量
if(isset($_SESSION['mscms__id']) && isset($_SESSION['mscms__cion'])){
$str  = str_replace("{mscms:cion}",$_SESSION['mscms__cion'],$str);
}else{
$str  = str_replace("{mscms:cion}",0,$str);
}

//网站部分链接
if(Web_Mode==3){
$str=str_replace('{mscms:gbooklink}','http://'.Web_Url.Web_Path.'gbook',$str);
}else{
$str=str_replace('{mscms:gbooklink}',site_url('gbook'),$str);
}
//搜索链接
$solink='';
if($dir!=''){
...
$str=str_replace('{mscms:solink}',$solink,$str);
$str=str_replace('{mscms:codes}',site_url('api/codes'),$str);
$str=str_replace('{mscms:userlink}',site_url('user'),$str);
$str=str_replace('{mscms:loginlink}',site_url('user/login'),$str);
$str=str_replace('{mscms:reglink}',site_url('user/reg'),$str);
$str=str_replace('{mscms:passlink}',site_url('user/pass'),$str);
$str=str_replace('{mscms:qqlink}',site_url('user/open/login/qq'),$str);
$str=str_replace('{mscms:weibolink}',site_url('user/open/login/weibo'),$str);
$str=str_replace('{mscms:kaixinlink}',site_url('user/open/login/kaixin'),$str);
$str=str_replace('{mscms:baidulink}',site_url('user/open/login/baidu'),$str);
$str=str_replace('{mscms:doubanlink}',site_url('user/open/login/douban'),$str);
$str=str_replace('{mscms:sohulink}',site_url('user/open/login/sohu'),$str);
$str=str_replace('{mscms:wangyilink}',site_url('user/open/login/netease'),$str);
$str=str_replace('{mscms:renrenlink}',site_url('user/open/login/renren'),$str);
$str=userurl($str);
return $str;
}

//组装SQL标签
public function mscms_sql($fields, $para, $str_arr, $label, $sorts='', $autoarr=array(), $cid=0, $sql='') {
...
//解析SQL标签
public function mscms_sql_to($fields, $para, $str_arr, $label, $str, $autoarr) {
$sql=$this->mscms_sql($fields, $para, $str_arr, $label, '', $autoarr);
...
$Data_Content.=$this->mscms_skins($fields,$str,$label,$row,$sorti,$autoarr);
$sorti++;
}
}
$str=str_replace($str_arr,$Data_Content,$str);	
...
//解析mscms标签
public function mscms_skins($field, $str, $label, $row, $sorti=1, $autoarr=array()) {
...
//判断是否嵌套二级
preg_match_all('/{mscmstype:([\S]+)\s+(.*?)}([\s\S]+?){\/mscmstype:\1}/',$label,$type_arr);
if(!empty($type_arr)){
for($i=0;$i<count($type_arr[0]);$i++){
$label=$this->mscms_sql_to($type_arr[1][$i],$type_arr[2][$i],$type_arr[0][$i],$type_arr[3][$i],$label,$row['id']);
}
}
unset($type_arr);
}
...
// 会员主页模板标签处理
public function mscms_web($web,$content,$str) {
$dir=MSCMS.'tpl/home/';
$newweb='';
if (is_dir($dir)){
if ($dh = opendir($dir)){
...
$pic=file_exists($filePath.'/preview.jpg')?Web_Path.'mscms/tpl/home/'.$con['dir'].'/preview.jpg':Web_Path.'packs/images/skins.jpg';
...
// php标签处理
public function mscms_php($php,$content,$str) {

...
class Denglu {
	...
	public function login($ac,$log_state=''){
        if(CS_Appmode==1){  //官方登录
              $log_url="http://denglu.msvod.cc/denglu?ac=".$ac."&appid=".CS_Appid."&redirect_uri=".$this->redirect_uri."&state=".$log_state."&getdate=".time();
	...
	public function callback($ac,$log_state=''){
        if(CS_Appmode==1){  //官方登录
              return $this->mscms_callback($ac,$log_state);
		}else{  //独立
			  $mode=$ac.'_callback';
	...
	//官方登录返回
    public  function mscms_callback($ac,$log_state='') {
    ...
    function get_url_contents($url,$post='',$type='get'){
		        ...
		        curl_setopt($ch, CURLOPT_USERAGENT, 'mscms');
		        $result = curl_exec($ch);
                curl_close($ch);
                return $result;
    }

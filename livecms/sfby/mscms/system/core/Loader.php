...
class CI_Loader {
	...
	public function model($model, $name = '', $db_conn = FALSE)
	{
		...
		foreach ($this->_ci_model_paths as $mod_path)
		{
			...
			if ( ! file_exists(MSCMS.'app/models/'.$path.$model.'.php') && ! file_exists($mod_path.'models/'.$path.$model.'.php'))
			{
				continue;
			}
	...
	public function helper($helpers = array())
	{
		foreach ($this->_ci_prep_filename($helpers, '_helper') as $helper)
		{
			...
			foreach ($this->_ci_helper_paths as $path)
			{
				...
				{
                      $helper_path=MSCMS.'app/helpers/'.$helper.'.php';
				}
	...
	protected function _ci_load($_ci_data)
	{
		...
		if ($_ci_return === TRUE)
		{
			...
			if(strpos($buffer,cs_base64_decode('PGhlYWQ-')) !==FALSE ){
                ...
                $buffer=$view[0].cs_base64_decode('PG1ldGEgbmFtZT0iZ2VuZXJhdG9yIiBjb250ZW50PSJ2aWRlbyIgLz4KPG1ldGEgbmFtZT0iYXV0aG9yIiBjb250ZW50PSJ2b2QiIC8+CjxtZXRhIG5hbWU9ImNvcHlyaWdodCIgY29udGVudD0ibW92IiAvPgo8L2hlYWQ+').$view[1];
			}
			@ob_end_clean();
			return $buffer;
		}
	...
	protected function _ci_load_class($class, $params = NULL, $object_name = NULL)
	{
		...
		foreach (array(ucfirst($class), strtolower($class)) as $class)
		{
			...
			foreach ($this->_ci_library_paths as $path)
			{
				...
				{
					$filepath = MSCMS.'app/libraries/'.$subdir.$class.'.php';
				}
		        ...
		        if (is_file($dir_file)) {
			        ...
			        if (is_array($plub)) {
						  ...
						  //前台视图
							  ...
							  if(defined('HOMEPATH')){
								   if($ulog==1) $path = str_replace("\\", "/", MSCMS);
								   if(!empty($skins)){
			                           $dirs='tpl/home/'.$skins;
								   }else{

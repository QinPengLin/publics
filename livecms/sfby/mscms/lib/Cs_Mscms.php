<?php
/**
 * @Mscms v4.0 open source management system
...
if(defined('MOBILE')){
    if(!defined('IS_ADMIN') && Mobile_Is==1){
          if(Home_Ym==0 && Mobile_Url!='' && $_SERVER['HTTP_HOST']!=Mobile_Url){
				$Web_Link="http://".str_replace('//','/',Mobile_Url.Web_Path.mscms_cur_url());
                header("location:".$Web_Link);exit;
		  }
    }
}else{
     $Web_Link="http://".str_replace('//','/',Web_Url.Web_Path.mscms_cur_url());
...
//判断版块绑定域名
$_ERYM=FALSE;
if(file_exists(MSCMS.'lib/Cs_Domain.php')){
    $_CS_Domain = require_once(MSCMS.'lib/Cs_Domain.php');
...
if(!defined('IS_ADMIN') && !defined('PLUBPATH')){
     ...
     if(!empty($SELF)){
            //手机访问
		    if(defined('MOBILE') && is_dir(FCPATH.'plugins/'.$SELF) && !file_exists(MSCMS.'app/controllers/home/'.$SELF.'.php')){
                  define('PLUBPATH', $SELF);
				  $app_folder = 'plugins/'.$SELF;
			//会员
			}elseif((defined('USERPATH') || defined('HOMEPATH')) && is_dir(FCPATH.'plugins/'.$SELF) && !file_exists(MSCMS.'app/controllers/user/'.$SELF.'.php')){
                  define('PLUBPATH', $SELF);
				  $app_folder = 'plugins/'.$SELF;
			}
	 }
     ...
     if(!defined('PLUBPATH') && defined('HOMEPATH')){
		    if(count($permarr)>2){
		        ...
		        if(defined('MOBILE') && is_dir(FCPATH.'plugins/'.$SELF) && !file_exists(MSCMS.'app/controllers/home/'.$SELF.'.php')){
				      ...
				      $app_folder = 'plugins/'.$SELF;
			    //会员
			    }elseif((defined('USERPATH') || defined('HOMEPATH')) && is_dir(FCPATH.'plugins/'.$SELF) && !file_exists(MSCMS.'app/controllers/home/'.$SELF.'.php')){
                      define('PLUBPATH', $SELF);
				      $app_folder = 'plugins/'.$SELF;
			    }
		    }
